...
def score_at(connection, match_id, innings_no, end_ball):
    row = connection.execute(
        "SELECT COALESCE(SUM(runs),0),COALESCE(SUM(wickets),0) FROM deliveries WHERE match_id=? AND innings_no=? AND ball_pos<=?",
...
def calculate_result(connection, league, innings_no, current_ball, current_runs, current_wickets, session_over, threshold):
    ...
    return {
        ...
        "low": ordered[max(0,int(len(ordered)*.1)-1)],
        "high": ordered[max(0,int(len(ordered)*.9)-1)]
    }
...
with st.sidebar:
    ...
    session_over = st.number_input(
        "Session Over", 1, 20,
        int(st.session_state.session_over), 1,
        key="session_over_input"
    )

    target = st.number_input(
        "Target Runs", 0, 400,
        int(st.session_state.target), 1,
        key="target_input"
    )
    ...
    start_runs = st.number_input(
        "Start Runs", 0, 400, 16, 1,
        key="start_runs"
    )

    start_wickets = st.number_input(
        "Start Wickets", 0, 10, 1, 1,
        key="start_wickets"
    )
...
st.markdown(
    f"<div class='card'><h3>Current Live Score</h3><h2>{runs}/{wickets}</h2><p class='small'>Over/Ball: {display_over(balls)} • Target: {target or 'Not set'} • Session over: {session_over} • Last: {st.session_state.last or '—'}</p></div>",
    unsafe_allow_html=True
)

st.subheader("Ball-by-Ball Update")

for index, (label, run_value, wicket) in enumerate([
    ("Dot",0,False),
    ("1 Run",1,False),
    ("2 Runs",2,False),
    ("3 Runs",3,False),
    ...
    with st.columns(4)[index%4]:
        if st.button(label, use_container_width=True, key=f"ball_{index}"):
            ...
            if label == "Undo" and st.session_state.undo:
                st.session_state.runs, st.session_state.wickets, st.session_state.balls, _ = st.session_state.undo.pop()
                st.session_state.last = "Undo"

            elif label != "Undo":
                st.session_state.undo.append(
                    (runs,wickets,balls,st.session_state.last)
                )
...
st.markdown(
    f"<div class='session-box'><h3>Session</h3><h2>{int(st.session_state.low)}-{int(st.session_state.high)}</h2><p class='small'>Expected: {float(st.session_state.expected):.1f} • Over: {session_over} • Target: {target or 'Not set'} • Samples: {samples}</p></div>",
    unsafe_allow_html=True
)
...
if st.session_state.analysis is None:
    ...
    css = "yes" if label == "YES" else "no"

    st.subheader("VasuDev Result")

    st.markdown(
        f"<div class='{css}'><h1>{label} — {max(yes,no):.1f}%</h1><p>Session line: <b>{int(st.session_state.low)}-{int(st.session_state.high)}</b> • {data['samples']} similar states</p></div>",
        unsafe_allow_html=True
    )
