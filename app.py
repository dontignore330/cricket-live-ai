...
try:
    if league != "IPL":
        ...
        selected_db, built_now = DB_PATHS["IPL"], False
...
with c2:
    ...
    bowling = st.selectbox(
        "Bowling Team",
        bowling_options,
        index=bowling_options.index(default_bowl)
    )
...
if st.button(
    ...
    if not cand.empty:
        ...
        range_low = float(
            cand.future_score.quantile(.10)
        )

        range_high = float(
            cand.future_score.quantile(.90)
        )
    ...
    if win_samples:
        ...
        st.info(
            "Not enough historical match-result "
    ...
    if session_samples:
        ...
        st.info(
            "Not enough historical continuation "
    ...
    def historical_extra_results(
        ...
        """
        Fast historical average/trend calculation for
        the selected window.
        """
    ...
    with st.expander(
        ...
        if session_samples:
            ...
            st.write(
                ...
                f"**{session_yes:.1f}% YES** because "
                f"the selected target was reached in "
                f"**{session_target_yes:,} of "
                f"{session_samples:,}** cases; "
                f"**{session_target_no:,}** did not "
                f"reach it."
            )


            st.write(
                f"• The result is **weighted**, so closer "
                f"matches to the current team, ground, "
                f"score, wickets and ball position have "
                f"more influence than weaker matches."
            )


            st.write(
                f"• Historical average added "
                f"**+{safe_round(extra['avg_added'])} runs**, "
                f"giving a projected score of "
                f"**{safe_round(extra['projected_score'])}** "
                f"by "
                f"{over_ball_from_balls(target_ball)}."
            )
            ...
            if extra.get("trend") is not None:
                ...
                st.write(
                    ...
                    f"**{safe_round(abs(tr['change_value']))} "
                    f"runs per 6 balls** around "
                    f"{over_ball_from_balls(tr['change_ball'])}."
                )
        ...
        if win_samples:
            ...
            st.write(
                ...
                f"**{win_samples:,}** similar match states; "
                f"the percentage is not manually increased."
            )


        st.write(
            "VasuDev does not manually increase a "
            "probability to make it look better. "
            "Advanced validation/backtesting is kept "
            "separate from the live result."
        )
