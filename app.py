...
if built_now:
    ...
    st.caption(
        f"{league} data prepared automatically and "
        "opened read-only for analysis."
    )
...
def balls_from_over_ball(value):
    ...
    # Cricket over.ball is NOT decimal math:
    # each over has exactly 6 legal balls.
    # We accept whole-over states and legal balls 1-6.
...
def over_ball_from_balls(balls):
    ...
    # Internal ball count is one-based within each over:
    # 4.6 is ball 30,
    # next legal delivery is 5.1.
...
def valid_over_ball_options(max_over=20):
    ...
    # Offer only legal delivery positions.
    # 0.0 means before first ball.
...
def load_history(selected_league):
    ...
    # Historical files can contain illegal-delivery labels
    # such as 4.7 or 4.10.
    # Those labels are valid historical sequence markers.
    # Strict validation is only for user input.
...
def similarity_candidates(
    ...
    if cur.empty:
        ...
        # Small ball-position window
        # prevents sparse exact positions
        # from killing the result.
    ...
    # Never use a state from after target.
    ...
    # Score/wicket neighborhood first;
    ...
    # Team / ground match strength.

    selected["team_score"] = (
        (
            selected.batting_team
            == batting
        ).astype(float)
        * 1.0
        +
        (
            selected.bowling_team
            == bowling
        ).astype(float)
        * 0.8
    )
...
def add_future_scores(
    ...
    for idx, row in candidates.iterrows():
        ...
        # Prefer state exactly at target;
        # otherwise last score at/before target.
...
def backtest_session_and_win(
    ...
    """
    Time-safe historical backtest:
    each test state excludes its own match.
    """
    ...
    # Keep states with meaningful future horizon.
...
with st.expander(
    "🧪 VasuDev Historical Backtest",
    expanded=False
):

    st.caption(
        "This test uses past match states and excludes "
        "the same match from its comparison pool. "
        "It tests the current target over the current "
        "ball horizon; it is a directional backtest, "
    ...
    if st.button(
        ...
        if bt is None:
            ...
            st.warning(
                "Not enough historical data for a backtest."
            )
            ...
            if (
                ...
                st.write(
                    "Average historical WIN probability "
                    "across tested states: "
                    f"**{bt['win_avg_probability']:.1f}%**"
                )

            st.info(
                "Backtest accuracy is measured only on "
                "held-out historical states. It is a "
                "measurement of past performance, not a "
            ...
            if (
                ...
                st.markdown(
                    "#### 🎯 Session Probability Calibration"
                )
...
if st.button(
    ...
    st.write(
        f"Current: **{current_runs}/{wickets} "
        f"at {over_ball_from_balls(current_ball)}** "
        f"→ Future: **{over_ball_from_balls(target_ball)}** "
        f"→ Target: **{target_runs}**"
    )

    # Existing eventual-match-result style.
    ...
    st.markdown(
        "### 🏆 Historical Team Winning / Loss Result"
    )
    ...
    if (
        ...
        if len(valid_result):
            ...
            st.write(
                f"**{batting}:** "
                f"{win_pct:.1f}% historical win frequency "
                f"• **Loss:** {loss_pct:.1f}% "
                f"• **Other:** {other_pct:.1f}%"
            )

            st.caption(
                "This is based on historical match states "
                "similar to the current score, wickets, "
                "ball position, teams and ground. "
                "It is historical frequency, not a guarantee."
            )

        else:

            st.info(
                "Similar states were found, but they do not "
                "contain a usable final match result."
            )

    else:

        st.info(
            "No usable historical match-result sample was found."
        )
    ...
    if not cand.empty:
        ...
        if cand.empty:
            ...
            st.info(
                "Historical states were found, but not enough "
                "of them continued to the selected future point."
            )
            ...
            st.write(
                f"**Question:** Similar historical situations "
                f"— did the batting team reach **{target_runs} "
                f"runs by {over_ball_from_balls(target_ball)}**?"
            )
            ...
            st.write(
                f"Expected score at "
                f"{over_ball_from_balls(target_ball)}: "
                f"**{safe_round(avg_score)} runs** "
                f"• Historical 10–90% range: "
                f"**{safe_round(q10)}–{safe_round(q90)}**"
            )

            st.caption(
                f"Similarity engine: {method}. "
                "Ground, team, score, wickets and ball "
                "position are weighted; broader selected-league "
                "data is used when exact situations are sparse."
            )

            if len(cand) < 30:

                st.warning(
                    "Small historical sample: treat this result "
                    "as low-data historical evidence."
                )

            elif len(cand) < 100:

                st.info(
                    "Moderate historical sample: the result is "
                    "based on similar situations, not exact duplicates."
                )

            else:

                st.success(
                    "Good historical sample size for this situation."
                )

    else:

        st.info(
            "No exact match was required, but the database "
            "could not find a usable historical continuation "
            "for this future point. Try a later future point "
            "or another IPL situation."
        )


    st.markdown(
        "### 🧩 How VasuDev handles rare situations"
    )

    st.write(
        "The system does not depend on one exact historical "
        "match from the selected league. It first uses close "
        "score/wicket/ball situations and gives extra weight "
        "to the selected ground and teams. If the exact "
        "combination is rare, it automatically broadens to "
        "similar IPL situations instead of simply showing "
        "Data Not Found."
    )

    st.caption(
        "Player-level adjustment is reserved for the next "
        "data layer because the current cricket_history.db "
        "does not contain the current playing XI/player-at-ball "
        "fields. The present engine therefore does not invent "
        "player information."
    )
