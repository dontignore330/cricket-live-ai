...
def _json_match_to_sqlite(db_path, league, zip_path):
    ...
    try:
        ...
        out.execute(
            ...
            "ON deliveries(league, innings_no, ball_no)"
        )
...
def ensure_bigbash_db_cached(league):
...
@st.cache_resource(show_spinner=False)
def get_db_connection_cached(db_path_str, db_mtime):
...
def get_database_counts(db_path_str, db_mtime):
    db_path = Path(db_path_str)
...
try:
    if league == "IPL":
        ...
        db_path_str, built_now = ensure_bigbash_db_cached(league)
        selected_db = Path(db_path_str)

except Exception as exc:
    st.error(str(exc))
    st.info(
        "IPL remains available. Reload after the hosting service "
        "has internet access to prepare Big Bash data."
    )
    st.stop()
...
db_mtime = selected_db.stat().st_mtime
...
try:
    match_count, delivery_count = get_database_counts(
        str(selected_db),
        db_mtime,
    )
...
def load_history(
    ...
    df["ball_pos"] = df["ball_no"].apply(
        historical_ball_position
    )
...
def get_teams_and_venues(
    ...
    teams = [
        x for x in teams_df["batting_team"].tolist()
        if x not in (None, "")
    ]

    venues = [
        x for x in venues_df["venue"].tolist()
        if x not in (None, "")
    ]
...
teams, venues = get_teams_and_venues(
    str(selected_db),
    db_mtime,
    league,
)
...
preferred_bat = preferred_bat_map.get(league)
default_bat = (
    preferred_bat
...
if st.session_state.get("batting_team") not in teams:
    st.session_state.batting_team = default_bat


bowling_options_for_state = [
    x
    for x in teams
    if x != st.session_state.batting_team
]

if not bowling_options_for_state:
    bowling_options_for_state = teams


preferred_bowl = preferred_bowl_map.get(league)

default_bowl = (
    preferred_bowl
    if preferred_bowl in bowling_options_for_state
    else bowling_options_for_state[0]
)

if (
    st.session_state.get("bowling_team")
    not in bowling_options_for_state
):
    st.session_state.bowling_team = default_bowl


if st.session_state.get("venue") not in venues:
    st.session_state.venue = venues[0]

if st.session_state.get("innings_label") not in [
    "1st Innings",
    "2nd Innings",
]:
    st.session_state.innings_label = "1st Innings"

if (
    st.session_state.get("current_over")
    not in VALID_CURRENT_POINTS
):
    st.session_state.current_over = "3.1"

if (
    st.session_state.get("future_over")
    not in VALID_FUTURE_POINTS
):
    st.session_state.future_over = "7.1"

if "current_runs" not in st.session_state:
    st.session_state.current_runs = 16

if "wickets" not in st.session_state:
    st.session_state.wickets = 1

if "target_runs" not in st.session_state:
    st.session_state.target_runs = 50

if st.session_state.get("match_format") not in ["T20"]:
    st.session_state.match_format = "T20"
...
with st.form("vasudev_analysis_form", clear_on_submit=False):
    ...
    st.info(
        ...
        f"**Balls remaining:** {remaining} • "
        f"**Target:** {target_runs} runs"
    )
...
def add_future_scores(
    ...
    base = candidates.reset_index(
        drop=True
    ).copy()

    base["candidate_id"] = np.arange(
        len(base),
        dtype=np.int32,
    )
    ...
    merged = merged[
        (merged.ball_pos > merged.start_ball)
        & (merged.ball_pos <= target_ball)
    ]
...
def historical_extra_results(
    ...
    if not future.empty:
        ...
        runs_matrix = future.pivot_table(
            index="case_id",
            columns="ball_pos",
            values="runs",
            aggfunc="sum",
        ...
        presence_matrix = future.assign(
            delivery_present=1
        ).pivot_table(
            index="case_id",
            columns="ball_pos",
            values="delivery_present",
            aggfunc="sum",
            fill_value=0,
        )

        runs_matrix = runs_matrix.reindex(
            columns=ball_range,
            fill_value=0,
        )

        presence_matrix = (
            presence_matrix.reindex(
                columns=ball_range,
                fill_value=0,
            )
        )
        ...
        valid_rolls = rolling_runs.where(
            rolling_presence > 0
        )
        ...
        trend = pd.DataFrame(
            {
                "ball": avg_series.index.astype(int),
                "avg_6ball_runs": avg_series.values,
            }
        )
        ...
        if not trend.empty:
            ...
            if len(trend) > 1:
                ...
                if abs(up_change) >= abs(
                    ...
                    change_value = up_change
                else:
                    direction = "decrease"
                    change_ball = int(
                        biggest_down["ball"]
                    )
                    change_value = down_change
...
with st.expander(
    ...
    if st.button(
        ...
        st.session_state.validation_result = bt
...
if analyze_clicked:
    ...
    # Clear old validation result only when a new analysis is made.
    ...
    if not cand.empty:
        ...
        session_target_yes = int(
            cand["hit"].sum()
        )

        session_target_no = int(
            len(cand)
            - session_target_yes
        )
    ...
    st.session_state.last_analysis = {
        ...
        "session_target_yes": session_target_yes,
        "session_target_no": session_target_no,
...
if result is not None:
    ...
    league_r = result["league"]
    batting_r = result["batting"]
    bowling_r = result["bowling"]
    venue_r = result["venue"]
    innings_label_r = result["innings_label"]

    current_ball_r = result[
        "current_ball"
    ]

    current_runs_r = result[
        "current_runs"
    ]
    ...
    target_ball_r = result[
        "target_ball"
    ]

    target_runs_r = result[
        "target_runs"
    ]
    ...
    win_samples_r = result[
        "win_samples"
    ]

    session_yes_r = result[
        "session_yes"
    ]
    ...
    session_samples_r = result[
        "session_samples"
    ]

    session_target_yes_r = result[
        "session_target_yes"
    ]

    session_target_no_r = result[
        "session_target_no"
    ]

    expected_score_r = result[
        "expected_score"
    ]
        ...
        st.markdown(
            ...
            <div class="{win_box}">
                ...
                <p class="small">
                    ...
                    {win_samples_r:,} similar match states •
                    Reliability:
                    {reliability(win_samples_r)}
                </p>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    if session_samples_r:

        label = (
            "YES"
            if session_yes_r >= session_no_r
            else "NO"
        )
        ...
        st.markdown(
            ...
            <div class="{box}">
                ...
                <p>
                    ...
                    <b>{target_runs_r}</b>
                    by
                    <b>{over_ball_from_balls(target_ball_r)}</b>
                    • {remaining_r} balls remaining
                </p>

                <p class="small">
                    Historical probability •
                    {session_samples_r:,} similar situations •
                    Reliability:
                    {reliability(session_samples_r)}
                </p>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    # =====================================================
    # HISTORICAL AVERAGE + SCORING CHANGE
    # =====================================================
    ...
    with st.expander(
        ...
        st.write(
            f"**Current:** "
            f"{current_runs_r}/{wickets_r} at "
            f"{over_ball_from_balls(current_ball_r)} "
            f"→ **Future:** "
            f"{over_ball_from_balls(target_ball_r)} "
            f"→ **Target:** "
            f"{target_runs_r}"
        )
        ...
        if session_samples_r:
            ...
            st.write(
                f"YES: **{session_yes_r:.1f}%** "
                f"• NO: **{session_no_r:.1f}%**"
            )

            if (
                expected_score_r is not None
                and range_low_r is not None
                and range_high_r is not None
            ):
                st.write(
                    f"Expected score at future point: "
                    f"**{safe_round(expected_score_r)}** "
                    f"• Historical 10–90% range: "
                    f"**{safe_round(range_low_r)}–"
                    f"{safe_round(range_high_r)}**"
                )

            st.caption(
                f"Similarity: {method_r}. "
                f"Team, ground, score, wickets and ball "
                f"position are weighted; broader {league_r} "
                f"data is used when exact situations are sparse."
            )
            ...
            if extra_r is not None:
                ...
                st.write(
                    f"• **{session_samples_r:,}** "
                    f"similar historical situations were "
                    f"used from **{extra_r['match_count']:,} "
                    f"matches**."
                )

            st.write(
                f"• The historical calculation gave "
                f"**{session_yes_r:.1f}% YES** because "
                f"the selected target was reached in "
                f"**{session_target_yes_r:,} of "
                f"{session_samples_r:,}** cases; "
                f"**{session_target_no_r:,}** did not reach it."
            )

            st.write(
                "• The result is **weighted**, so closer "
                "matches to the current team, ground, score, "
                "wickets and ball position have more influence "
                "than weaker matches."
            )
            ...
            if extra_r is not None:
                ...
                st.write(
                    f"• Historical average added "
                    f"**+{safe_round(extra_r['avg_added'])} "
                    f"runs**, giving a projected score of "
                    f"**{safe_round(extra_r['projected_score'])}** "
                    f"by "
                    f"{over_ball_from_balls(target_ball_r)}."
                )

                if extra_r.get("trend") is not None:
                    ...
                    st.write(
                        f"• Scoring trend "
                        f"{direction_word} by about "
                        f"**{safe_round(
                            abs(tr['change_value'])
                        )} runs per 6 balls** "
                        f"around "
                        f"**{over_ball_from_balls(
                            tr['change_ball']
                        )}**."
                    )
        ...
        if win_samples_r:
            ...
            st.write(
                f"WIN: **{win_pct_r:.1f}%** "
                f"• LOSS: **{loss_pct_r:.1f}%** "
                f"• Other/Tie: **{other_pct_r:.1f}%**"
            )

            st.write(
                f"• WIN/LOSS is calculated separately "
                f"from historical match outcomes using "
                f"**{win_samples_r:,}** similar match states; "
                f"the percentage is not manually increased."
            )
